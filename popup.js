class PopupManager {
  ...
  async loadSettings() {
    ...
    // Log the actual storage value to debug
    console.log('🛡️ Popup: Storage enabled value:', result.enabled);
    ...
    if (result.enabled === undefined || result.enabled === null) {
      ...
      console.log('🛡️ Popup: No enabled value in storage, setting default to true');
    } else {
      // Ensure it's a boolean
      this.isEnabled = Boolean(result.enabled);
    }
    
    console.log('🛡️ Popup: Final isEnabled state:', this.isEnabled);
    ...
    if (resetRulesBtn) {
      resetRulesBtn.addEventListener('click', async () => {
        if (confirm('Reset all rules to default? This will remove any custom rules.')) {
          console.log('🛡️ Resetting rules to default...');
  ...
  setupPagination() {
    ...
    if (rulesNextBtn) {
      rulesNextBtn.addEventListener('click', () => {
        console.log('🛡️ Rules next button clicked');
        console.log('🛡️ Filtered rules:', this.filteredRules?.length || 0);
        console.log('🛡️ Current page:', this.rulesPage);
        ...
        console.log('🛡️ Total pages:', totalPages);
        
        if (this.rulesPage < totalPages) {
          this.rulesPage++;
          console.log('🛡️ Moving to page:', this.rulesPage);
          this.displayRulesPage();
        }
      });
    }
  ...
  updateToggleStates() {
    const enableToggle = document.getElementById('enableToggle');
    if (enableToggle) {
      console.log('🛡️ Popup: Setting toggle checkbox to:', this.isEnabled);
      enableToggle.checked = this.isEnabled;
      // Double-check the checkbox state
      console.log('🛡️ Popup: Toggle checkbox is now:', enableToggle.checked);
    }
  ...
  async toggleExtension() {
    ...
    console.log('🛡️ Extension toggle changed to:', this.isEnabled);
    await chrome.storage.sync.set({ enabled: this.isEnabled });
    
    if (this.isEnabled) {
      console.log('🛡️ Extension enabled, triggering reanalysis...');
      ...
      if (this.isValidUrl(tab.url)) {
        try {
          ...
          console.log('🛡️ Content script not available, will inject it');
        }
      }
      ...
      console.log('🛡️ Extension disabled, clearing results and showing disabled state');
  ...
  async loadResults() {
    console.log('🛡️ Popup: loadResults called, isEnabled:', this.isEnabled);
    
    if (!this.isEnabled) {
      console.log('🛡️ Popup: Extension is disabled, showing disabled state');
      this.showDisabledState();
      return;
    }
    
    console.log('🛡️ Popup: Extension is enabled, proceeding to load results...');
    ...
    if (!this.isValidUrl(tab.url)) {
      console.log('🛡️ Popup: Invalid URL for content script:', tab.url);
    ...
    try {
      ...
      console.log('🛡️ Popup: Requesting results for tab:', tab.id, 'URL:', tab.url);
      const response = await chrome.runtime.sendMessage({
        action: 'getTabResults',
        tabId: tab.id
      });
      
      console.log('🛡️ Popup: Received response:', response);
      this.currentResults = (response && response.results) ? response.results : [];
      this.currentTabId = tab.id;
      this.currentTabUrl = tab.url;
      console.log('🛡️ Popup: Current results count:', this.currentResults.length);
      ...
      if (this.currentResults.length === 0) {
        ...
        if (this.currentResults.length > 0) {
          console.log('🛡️ Popup: Got results after ensuring content script');
        } else {
          console.log('🛡️ Popup: Still no results after ensuring content script');
        }
      }
  ...
  async ensureContentScriptAndAnalyze(tab) {
    try {
      console.log('🛡️ Popup: Ensuring content script for tab:', tab.id);
      
      // First, try to ping the existing content script
      let contentScriptExists = false;
      try {
        const pingResponse = await chrome.tabs.sendMessage(tab.id, { action: 'ping' });
        console.log('🛡️ Popup: Content script is responding:', pingResponse);
        contentScriptExists = true;
      } catch (error) {
        console.log('🛡️ Popup: Content script not responding, injecting it...');
      }
      ...
      if (!contentScriptExists) {
        ...
        console.log('🛡️ Popup: Content script injected, waiting for initialization...');
        
        // Wait for content script to be ready (it will send contentScriptReady message)
        // Use a longer timeout for initial load
        await new Promise(resolve => setTimeout(resolve, 2000));
      } else {
        // Content script exists but no results, trigger reanalysis
        console.log('🛡️ Popup: Triggering reanalysis...');
        await chrome.tabs.sendMessage(tab.id, { action: 'reanalyze' });
      ...
      console.log('🛡️ Popup: Results after ensuring content script:', response);
      this.currentResults = (response && response.results) ? response.results : [];
      
      // If still no results, try one more time with a longer wait
      if (this.currentResults.length === 0) {
        console.log('🛡️ Popup: No results yet, waiting a bit more...');
        ...
        console.log('🛡️ Popup: Final results:', this.currentResults.length);
      }
    } catch (error) {
  ...
  async createSimpleDetectionElement(result) {
    ...
    if (result.icon && result.icon.startsWith('custom_icon_')) {
      ...
      // Try to get icon based on detector name if icon property is missing
      let iconFile = result.icon;
      if (!iconFile || iconFile === 'custom.svg') {
        // Map common detector names to their icon files
        const nameToIcon = {
    ...
    detectionHeader.innerHTML = `
      <div class="detection-info">
        <div class="detection-name-container">
          ...
          <div class="detection-name-simple">${result.name}</div>
        </div>
  ...
  async loadAdvancedTab() {
    console.log('🛡️ Loading Advanced tab');
    ...
    if (tabs.length > 0) {
      ...
      console.log('🛡️ Advanced tab initial check, results:', response?.results?.length || 0);
      
      // If no results, try to trigger content script to send them
      if (!response || !response.results || response.results.length === 0) {
        console.log('🛡️ Advanced: No results available, requesting from content script');
        try {
          await chrome.tabs.sendMessage(tabs[0].id, { action: 'getResults' });
          // Wait a bit for the content script to respond
          await new Promise(resolve => setTimeout(resolve, 200));
        } catch (e) {
          console.log('🛡️ Advanced: Could not request results from content script:', e);
        }
      }
    }
  ...
  initializeCaptureTerms() {
    ...
    console.log('🛡️ Initializing capture terms - only setting up event listeners');
  ...
  async initializeCaptureMode() {
    ...
    if (response && response.active) {
      ...
      console.log('🛡️ Capture mode is still active for this tab');
    } else {
      this.isCapturing = false;
      console.log('🛡️ Capture mode is not active for this tab');
    }
    
    // Setup event listeners
    const startBtn = document.getElementById('startCaptureBtn');
    if (startBtn) {
      console.log('🛡️ Setting up Start Capturing button event listener');
      startBtn.addEventListener('click', () => {
        console.log('🛡️ Start Capturing button clicked!');
        this.startCapture();
      });
    } else {
  ...
  async loadDetectionResultsForModal() {
    try {
      ...
      if (response && response.results) {
        ...
        console.log('Loaded detection results for modal:', this.currentResults);
      }
    } catch (error) {
  ...
  addDetectionDataToModal() {
    ...
    console.log('Current detection results:', this.currentResults);
  ...
  loadDetectionsForCapture() {
    ...
    console.log('🛡️ Loading detections for capture, currentResults:', this.currentResults);
    ...
    if (!this.currentResults || !Array.isArray(this.currentResults)) {
      console.log('🛡️ No current results available for capture checkboxes');
    ...
    captchaDetections.forEach(detection => {
      ...
      detectionItem.addEventListener('click', () => {
        ...
        if (startBtnCurrent) {
          ...
          console.log('🛡️ Enabled Start Capturing button for:', detection.name);
        } else {
          console.log('🛡️ Warning: Start Capturing button not found');
        }
      });
  ...
  async startCapture() {
    ...
    console.log('🛡️ selectedDetections:', Array.from(this.selectedDetections));
    ...
    try {
      ...
      const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
      console.log('🛡️ Sending startCaptureMode message for tab:', tab.id);
      ...
      console.log('🛡️ Background script response:', response);
  ...
  displayCapturedParameters() {
    ...
    console.log('🛡️ Displaying captured parameters:', this.capturedParams?.length || 0);
  ...
  async loadCapturedParameters() {
    console.log('🛡️ Loading captured parameters...');
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    const response = await chrome.runtime.sendMessage({
      action: 'getCapturedParameters',
      tabId: tab.id
    ...
    console.log('🛡️ Captured parameters response:', response);
    
    if (response && response.captures && response.captures.length > 0) {
      console.log('🛡️ Found', response.captures.length, 'captures');
      this.capturedParams = response.captures;
      this.displayCapturedParameters();
    } else {
      console.log('🛡️ No captures found yet');
      this.capturedParams = [];
  ...
  loadExportTab() {
    ...
    console.log('Export tab loaded - Coming Soon');
  }
  ...
  async loadProviderButtons() {
    try {
      ...
      if (this.cachedProviders) {
        console.log('🛡️ Using cached providers');
      ...
      console.log('🛡️ Loaded and cached providers');
...
?>`;
  ...
  async loadBlacklistUI() {
    ...
    if (!container) {
      console.log('🛡️ Blacklist container not found, will load when settings modal opens');
      return;
    }
  ...
  async loadCustomRules() {
    try {
      ...
      if (needsReset) {
        console.log('Resetting rules with version', CURRENT_VERSION);
        this.customRules = [];
        await this.populateDefaultRules();
  ...
  async loadAdvancedResults() {
    try {
      const tabs = await chrome.tabs.query({ active: true, currentWindow: true });
      if (tabs.length === 0) {
        console.log('🛡️ Advanced: No active tabs found');
        return;
      }

      console.log('🛡️ Advanced tab requesting results for tab:', tabs[0].id);
      ...
      console.log('🛡️ Advanced tab received response:', response);
      console.log('🛡️ Advanced response results:', response?.results);
      
      if (response && response.results && Array.isArray(response.results)) {
        console.log('🛡️ Advanced: Processing', response.results.length, 'results');
        ...
        console.log('🛡️ Advanced: No results or invalid response structure');
  ...
  updateAdvancedSection(advancedResults) {
    ...
    if (!advancedSection || !advancedEmpty || !tableBody) {
      console.log('🛡️ Advanced: Missing DOM elements');
      return;
    }

    // Debug: Log what we received
    console.log('🛡️ updateAdvancedSection called with:', advancedResults);
    console.log('🛡️ Number of results:', advancedResults ? advancedResults.length : 'undefined/null');
    
    // Ensure advancedResults is an array
    if (!Array.isArray(advancedResults)) {
      console.log('🛡️ Advanced: Results is not an array, converting to empty array');
      advancedResults = [];
    }
    
    if (advancedResults.length > 0) {
      console.log('🛡️ First result structure:', advancedResults[0]);
    ...
    console.log('🛡️ Filtered reCAPTCHA results:', recaptchaResults.length, recaptchaResults);
    console.log('🛡️ All results for debugging:', advancedResults);
    ...
    console.log('🛡️ reCAPTCHA detected, showing capture interface');
    ...
    if (advancedEmpty) {
      ...
      console.log('🛡️ Hiding advancedEmpty - reCAPTCHA was detected');
    }
    ...
    if (advancedSection) {
      if (networkCapturedResults.length > 0) {
        ...
        console.log('🛡️ Shown capturedParameters section with', networkCapturedResults.length, 'captures');
      } else {
        // Hide the table - no captures yet
        advancedSection.style.display = 'none';
        advancedSection.style.visibility = 'hidden';
        console.log('🛡️ Hiding capturedParameters section - no captures yet');
      }
    }
    if (captureHeader) {
      captureHeader.style.display = 'block';
      captureHeader.style.visibility = 'visible';
      console.log('🛡️ Shown captureHeader');
    }
    if (triggerBadge) {
    ...
    chrome.storage.local.get(['captureTermsAccepted'], (result) => {
      if (result.captureTermsAccepted) {
        ...
        if (captureSelectionSection) {
          ...
          console.log('🛡️ Shown captureSelectionSection (terms accepted)');
        ...
        if (captureTermsSection) {
          ...
          console.log('🛡️ Shown captureTermsSection (terms not accepted)');
        }
        if (captureSelectionSection) {
  ...
  showAdvancedEmptyState() {
    console.log('🛡️ Showing Advanced empty state - No reCAPTCHA detected');
    ...
    if (captureSelectionSection) {
      captureSelectionSection.style.display = 'none';
      captureSelectionSection.style.visibility = 'hidden';
      console.log('🛡️ Hidden captureSelectionSection');
    }
    ...
    if (captureTermsSection) {
      captureTermsSection.style.display = 'none';
      captureTermsSection.style.visibility = 'hidden';
      console.log('🛡️ Hidden captureTermsSection');
    }
    if (captureHeader) {
      captureHeader.style.display = 'none';
      captureHeader.style.visibility = 'hidden';
      console.log('🛡️ Hidden captureHeader');
    }
    if (triggerBadge) {
      triggerBadge.style.display = 'none';
      triggerBadge.style.visibility = 'hidden';
    ...
    if (advancedEmpty) {
      ...
      console.log('🛡️ Shown advancedEmpty');
    }
  }
  ...
  async createCustomRuleElement(rule, index) {
    ...
    if (rule.icon && rule.icon.startsWith('custom_icon_')) {
      ...
      iconSrc = chrome.runtime.getURL(`icons/providers/${iconFile}`);
      console.log(`Rule: ${rule.name}, Icon file: ${iconFile}, URL: ${iconSrc}`);
    }
  ...
  async ensureAllRulesHaveIcons() {
    ...
    if (updated) {
      await chrome.storage.local.set({ customRules: this.customRules });
      console.log('Updated rules with missing icons');
    }
  }

  async populateDefaultRules() {
    try {
      console.log('🛡️ Starting to populate default rules...');
      
      // First load the detector index
      const indexResponse = await fetch(chrome.runtime.getURL('detectors/index.json'));
      const indexData = await indexResponse.json();
      console.log('🛡️ Index loaded with detectors:', Object.keys(indexData.detectors));
      ...
      for (const [detectorId, detectorConfig] of Object.entries(indexData.detectors)) {
        try {
          ...
          console.log(`🛡️ Loading detector: ${detectorId} from ${detectorConfig.file}`);
          ...
          console.log(`🛡️ Added rule: ${rule.name} (${rule.category})`);
        } catch (detectorError) {
      ...
      console.log(`🛡️ Total rules loaded: ${defaultRules.length}`);
      console.log('🛡️ Rules by category:', defaultRules.reduce((acc, rule) => {
